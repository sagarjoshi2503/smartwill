...
from _app.shared.constants import JWT_ALGORITHM, ROLE_ADMIN

URL = "/api/will/admin-wills"


def auth_headers(email="admin@lawfirm.com"):
    token = jwt.encode({"sub": email, "role": ROLE_ADMIN}, "test-secret-key", algorithm=JWT_ALGORITHM)
...
def test_returns_500_when_mongodb_uri_missing():
    ...
    try:
        ...
        res = TestClient(app).get(URL, headers=AUTH)

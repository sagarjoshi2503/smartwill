"""Plain-text port of web/src/data/faqData.tsx's FAQ_SECTIONS — the FAQ
page's actual copy, kept independent per this repo's "no shared code" rule
(see CLAUDE.md; rag/ never imports from web/). Answers here are stripped to
plain text (the web version's JSX bullet/paragraph formatting collapsed to
"- "-prefixed lines) since this file only feeds the embedding/keyword index
in faq_indexer.py, never gets rendered.

This is static site copy, not per-user data — when the FAQ page's wording
changes, update both this file and web/src/data/faqData.tsx by hand; there's
no build step that keeps them in sync automatically.
"""

FAQ_SECTIONS: list[dict] = [
    {
        ...
        "items": [
            ...
            {"q": "Who are the parties to a Will?", "a": "Testator — the person making the Will. Executor — the person appointed to carry out its terms (optional, but recommended). Beneficiaries — those who receive assets under it. Witnesses — a minimum of two, neither of whom can be a beneficiary."},
    ...
    },
    {
        ...
        "items": [
            {"q": "What is a Living Will?", "a": "A Living Will (legally called an Advance Medical Directive) is a document where you state your medical preferences in advance. It only comes into effect if you become terminally ill, unconscious, or incapacitated, and cannot speak for yourself. Why do people make one? Dignity: To avoid being kept artificially alive on machines when there is zero chance of recovery. Family Peace: To prevent family members from having to make painful, guilt-ridden decisions about shutting off machines. Financial Protection: To stop unnecessary ICU and ventilator costs from draining family savings."},
            {"q": "How does a Living Will work, in simple terms?", "a": "1. You write and sign it while you are healthy and of sound mind. 2. Two witnesses sign it with you. 3. A Notary or Gazetted Officer attests it. 4. A copy goes to a Designated Local Officer (like the Mamlatdar/Collectorate) for safe custody. 5. Activation: If you ever end up in an irreversible, comatose state, two independent hospital medical boards review your case and confirm that treatment is futile before honoring your directive."},

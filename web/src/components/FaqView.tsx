    ...
    <div className="fade-in">
      <section className="bg-slate-50 py-14 px-5 border-b border-slate-200">
        <div className="max-w-4xl mx-auto">
          <p className="text-[#4F9D33] tracking-[0.35em] uppercase text-xs mb-3">Support</p>
          <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 serif mb-5">FAQ</h1>
          <div className="bg-[#F3F7E7] border border-[#4F9D33]/25 rounded-xl p-3.5 text-xs text-[#2D6B1F] leading-relaxed">
          ...
          {FAQ_SECTIONS.map(section=>{
            ...
            return(
              <div key={key} id={`faq-${key}`}>
                <h3 className="text-slate-900 font-bold text-lg serif border-b-2 border-[#4F9D33] pb-2 mb-4">{section.title}</h3>
                <div className="apv-card overflow-hidden">
                  <button onClick={()=>setOpen(isOpen?null:key)}
                    ...
                    <Plus size={16} className={`text-[#4F9D33] shrink-0 transition-transform ${isOpen?"rotate-45":""}`}/>
                  </button>
                  {isOpen && (

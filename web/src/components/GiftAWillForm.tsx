  ...
  if(successCode) {
    return(
      <div className="bg-white p-6 rounded-2xl border border-[#E5E8E3] text-center fade-in">
        <div className="w-14 h-14 bg-[#F3F7E7] rounded-full flex items-center justify-center mx-auto mb-4"><Gift size={26} className="text-[#4F9D33]"/></div>
        ...
        <div className="my-3 py-3 px-4 bg-[#F3F7E7] border border-dashed border-[#4F9D33] rounded-xl text-lg font-bold tracking-widest text-[#2D6B1F] font-mono">{successCode}</div>
        ...
        <button onClick={resetForm} className="text-slate-400 hover:text-[#4F9D33] text-xs underline mt-4 inline-block">← Send another gift</button>
      </div>
    );
  }
  ...
  return(
    <div className="bg-white p-6 rounded-2xl border border-[#E5E8E3]">
      ...
      <div className="grid gap-2 mb-4">
        {GIFT_PLANS.map(p=>(
          <label key={p.amount}
            className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl border-[1.5px] cursor-pointer transition-colors ${amount===p.amount?"border-[#4F9D33] bg-[#F3F7E7]":"border-[#E5E8E3] hover:border-[#C7D94A]"}`}>
            ...
            <span className="font-bold text-[#4F9D33] text-sm">₹{p.amount.toLocaleString("en-IN")}</span>
          </label>
        ))}
      </div>
      <div className="mb-3">

    ...
    <div className="fade-in">
      <section className="bg-slate-50 py-14 px-5 border-b border-slate-200">
        <div className="max-w-4xl mx-auto">
          <p className="text-[#4F9D33] tracking-[0.35em] uppercase text-xs mb-3">What We Offer</p>
        ...
        <div className="max-w-5xl mx-auto divide-y divide-slate-200">
          {SERVICES.map(svc=>(
            <div key={svc.id} id={`svc-${svc.id}`} className="grid md:grid-cols-[0.9fr_1.4fr] gap-6 md:gap-10 py-10 first:pt-0 last:pb-0">
              <div>
                <span className="block text-[#4F9D33] tracking-[0.3em] uppercase text-[10px] font-bold mb-2">{svc.eyebrow}</span>
                <h3 className="text-slate-900 font-bold text-lg serif mb-2">{svc.title}</h3>
                <div className="text-[#4F9D33] font-semibold text-sm">{svc.price}</div>
              </div>
              <div>
                  ...
                  {svc.actions.map(a=>(
                    <button key={a.label} onClick={()=>runAction(a.kind, "faqSection" in a ? a.faqSection : undefined)}
                      className={a.kind==="contact"||a.kind==="faq"
                        ? "text-xs font-semibold px-4 py-2 rounded-full border border-slate-200 text-slate-700 hover:border-[#4F9D33]/40 hover:text-[#4F9D33] transition-all"
                        : "text-xs font-semibold px-4 py-2 rounded-full bg-[#4F9D33] hover:bg-[#2D6B1F] text-white transition-colors"}>
                      {a.label}
                    </button>
                  ))}
                </div>

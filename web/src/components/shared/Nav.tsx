...
export default function Nav({ onNext, onPrev }: { onNext?: () => void; onPrev?: () => void }) {
  return (
    <div className="flex gap-3 pt-2">
      ...
      {onNext && <button onClick={onNext} className="flex-1 bg-[#2F8132] hover:bg-[#c4934c] text-[#ffffff] py-2.5 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-1.5">Next<ChevronRight size={14}/></button>}
    </div>
  );
}

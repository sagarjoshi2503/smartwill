          ...
          <Page>
            <p className="text-justify mb-5">
              {deedWitnesses.map((w,i)=>(
                <span key={i}>
                  ({i===0?"ONE":"TWO"}) MR./MRS. <strong>{w.name||blank}</strong>, {w.parentRelation} <strong>{w.parentName||blank}</strong>, date of birth <strong>{formatDOB(w.dateOfBirth)}</strong>, <strong>{w.maritalStatus||blank}</strong>, <strong>{w.occupation||blank}</strong>, Indian National, resident of <strong>{w.address||blank}</strong>, having PAN Card No. <strong>{w.pan||blank}</strong> and Aadhaar Card No. <strong>{w.aadhaarNumber||blank}</strong>;<br/>
                </span>
              ))}
            </p>

          ...
          <Page>
            <p className="text-justify mb-5">
              {deedWitnesses.map((w,i)=>(
                <span key={i}>
                  ({i===0?"ONE":"TWO"}) MR./MRS. <strong>{w.name||blank}</strong>, {w.parentRelation} <strong>{w.parentName||blank}</strong>, <strong>{w.age||"___"}</strong> years of age, <strong>{w.maritalStatus||blank}</strong> (marital status), <strong>{w.occupation||blank}</strong> (occupation), Indian National (nationality), resident of <strong>{w.address||blank}</strong>, having PAN Card No. <strong>{w.pan||blank}</strong> and Aadhaar Card No. <strong>{w.aadhaarNumber||blank}</strong>;<br/>
                </span>
              ))}
            </p>

        ...
        <Clause title="EXECUTOR">
          {executor.wantsExecutor?(
            executor.executorType==="org"?(
              ...
              <p className="text-justify">I appoint <strong>{executor.name||"[Executor]"}</strong> ({executor.relation==="Other"?executor.relationOther:executor.relation}), ID: {executor.idType} {executor.idNumber||"[No.]"}, residing at {executor.address||"[Address]"}, as Sole Executor. They shall act <em>{executor.adminType==="jointly"?"jointly":"jointly and severally"}</em>.{executor.hasSubstitute&&executor.subName?` Substitute: ${executor.subName}.`:""}</p>
            )
          ):(

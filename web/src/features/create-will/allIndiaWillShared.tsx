    ...
    <>
      ...
      {entries.map((entry, i) => (
        <span key={i}>{entry.beneficiaryType === "org" ? (
          <><strong>{entry.orgName || BLANK}</strong>, Authorized Representative: <strong>{entry.orgRepName || BLANK}</strong>, Registration / Tax ID Number: <strong>{entry.orgRegNumber || BLANK}</strong>, Registered Office Address: <strong>{entry.orgAddress || BLANK}</strong></>
        ) : (

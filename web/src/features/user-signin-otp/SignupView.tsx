    ...
    <div className="fade-in min-h-[calc(100vh-58px)] bg-slate-100 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-14 h-14 bg-[#4F9D33]/15 border border-[#4F9D33]/20 rounded-3xl flex items-center justify-center mx-auto mb-4"><User size={22} className="text-[#4F9D33]"/></div>
        ...
        <div className="apv-card p-6 space-y-4">
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center shrink-0 mt-1 transition-all peer-focus-visible:ring-2 peer-focus-visible:ring-[#4F9D33] peer-focus-visible:ring-offset-2 ${signup.terms?"bg-[#4F9D33] border-[#4F9D33]":"border-slate-400"}`}>
              {signup.terms&&<Check size={10} className="text-[#ffffff]"/>}
            </div>
            <span className="text-slate-600 text-sm">{TERMS_AGREE_PREFIX}<span className="text-[#4F9D33]">{LBL_TERMS_OF_SERVICE}</span>{TERMS_AGREE_JOINER}<span className="text-[#4F9D33]">{LBL_PRIVACY_POLICY}</span></span>
          </label>

...
import { beneficiaryName, beneficiaryRelationLabel } from "./beneficiaryDisplay";
...
export const formatAllocCompact = (asset: AssetInstance, beneficiaries: Beneficiary[]): string => {
  if (!asset.allowSplit) {
    ...
    return s ? `${beneficiaryName(s)} (${beneficiaryRelationLabel(s)}) — 100%` : "[Not assigned]";
  }
  return Object.entries(asset.allocs).filter(([, v]) => v && parseFloat(v) > 0)
    .map(([id, pct]) => {
      const b = beneficiaries.find(x => String(x.id) === id);
      return b ? `${beneficiaryName(b)} — ${pct}%` : null;
...
export const formatAllocFull = (asset: AssetInstance, beneficiaries: Beneficiary[]): string => {
  if (!asset.allowSplit) {
    const s = beneficiaries.find(b => String(b.id) === asset.allocs.sole);
    return s ? `${beneficiaryName(s)} (${beneficiaryRelationLabel(s)}), 100%` : "[Beneficiary not selected]";
  }
  return Object.entries(asset.allocs).filter(([, v]) => v && parseFloat(v) > 0)
    .map(([id, pct]) => {
      const b = beneficiaries.find(x => String(x.id) === id);
      return b ? `${beneficiaryName(b)} (${beneficiaryRelationLabel(b)}) — ${pct}%` : null;
    }).filter(Boolean).join("; ") || "[Allocation not set]";
};
